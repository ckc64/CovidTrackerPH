...
import 'package:covidtrackerph/dashboardata/timelineservices.dart' as timelineservices;
...
final List dateDeathTimeline,dateRecoveredTimeLine,countRecoveredTimeline,countDeathTimeline;
  const Timeline({Key key, this.dateDeathTimeline, this.dateRecoveredTimeLine, this.countRecoveredTimeline, this.countDeathTimeline}) : super(key: key);
...
class _TimelineState extends State<Timeline> {
  ...
  _generateData(){
   ...
   for(int i = 0; i<widget.countRecoveredTimeline.length;i++){
     recoveredData.add(RecoveredTimelineCases(i,widget.countRecoveredTimeline[i]));
   }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        actions: <Widget>[
          IconButton(
            onPressed: ()=>Navigator.push(context, MaterialPageRoute(builder: (context)=>TimelineData())),
                      ...
                      children: <Widget>[
                        Text(
                            ...
                            ,style: TextStyle(
                              ...
                              Text(
                            'note* sometimes the update of this trend is delayed'
                            ,style: TextStyle(
                              fontSize: 13.0,
                              fontFamily: 'Montserrat-Regular',
                              color: Colors.red
      ...
      selectionModels: [
          ...
          changedListener: (charts.SelectionModel model) {
            if(model.hasDatumSelection){
                  ...
                  pointValue = model.selectedSeries[0].measureFn(model.selectedDatum[0].index).toString();
                    //pointValue = "1";
